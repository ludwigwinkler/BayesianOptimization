...
Furthermore, let $\mathcal{D} = (X, y) = \\{(x_i, y_i)\\}_{i=0}$


$\mathcal{D} = (X, y) = \\{(x_i, y_i)\\} $ hello $\{x\}_{i=0}^N$, hello $\{x\}_\{i=0\}^N$ with $X \in$ $\mathbb{R}^{N d}$ and $y \in \mathbb{R}^{N}$, be our training observations of the function $f$.
